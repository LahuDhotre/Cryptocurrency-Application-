...
const Navbar = () => {
    ...
    return (
        <div className='rounded-div flex items-center justify-between h-20 font-bold'>
            <Link to='/'>
                <h1 className='text-2xl'>CryptoCurrent</h1>
            </Link>
            <div className='hidden md:block'>
                <ThemeToggle />
            </div>
            {user?.email ? (<div><Link to='/account' className='p-4'>Account</Link><button onClick={handleSignOut}>Sign Out</button></div>) : (<div className='hidden md:block'>
            ...
            <div className='block md:hidden cursor-pointer z-10' onClick={handleNav}>
                {nav ? <IoMdClose size={25} /> : <GiHamburgerMenu size={25} />}
            </div>
            <div className={nav ? 'md:hidden fixed left-0 top-20 flex flex-col items-center justify-between w-full h-[90%] bg-primary ease-in duration-300 z-10' : 'fixed left-[-100%] top-20 h-[-90%] flex flex-col items-center justify-between ease-in duration-300'}>
                ...
                <div className='flex flex-col w-full p-4'>
                    <Link to='/signin'><button className='w-full my-2 p-3 bg-primary border border-secondary rounded-2xl shadow-xl' onClick={handleNav}>Sign In</button></Link>
                    <Link to='/signup'><button className='w-full my-2 p-3 bg-button text-btnText rounded-2xl shadow-xl' onClick={handleNav}>Sign Up</button></Link>
                </div>
            </div>
        </div>
    )
